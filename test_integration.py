    ...
    try:
        ...
        with closing(socket.create_connection(("127.0.0.1", 29944), timeout=5)) as sock:
            ...
            if servercount == 0:
                ...
                print(
                    f"✗ New-style list test FAILED (expected 0, got {servercount} servers)"
                )
                return False
    except Exception as e:
...
def test_server_registration():
    ...
    try:
        ...
        kvpairs.append(
            bytes([4]) + b"name" + struct.pack(">H", len(name_bytes)) + name_bytes
        )
        ...
        kvpairs.append(
            bytes([4]) + b"game" + struct.pack(">H", len(game_bytes)) + game_bytes
        )
        ...
        kvpairs.append(
            bytes([11])
            + b"protocol_id"
            + struct.pack(">H", len(protocol_id_bytes))
            + protocol_id_bytes
        )
        ...
        with closing(socket.create_connection(("127.0.0.1", 29944), timeout=5)) as sock:
            ...
            if servercount == 1:
                ...
                if slots == 8 and players == 2 and bots == 0:
                    print(
                        "✓ Server registration test PASSED (server registered and found)"
                    )
                    return True
                else:
                    print(
                        f"✗ Server registration test FAILED (wrong server data: slots={slots}, players={players}, bots={bots})"
                    )
                    return False
            else:
                print(
                    f"✗ Server registration test FAILED (expected 1 server, got {servercount})"
                )
                return False
...
def test_legacy_protocol():
    ...
    try:
        ...
        if not connection_received.wait(timeout=3.0):
            print(
                "✗ Legacy protocol test FAILED (lobby didn't connect for reachability check)"
            )
            return False
        ...
        with closing(socket.create_connection(("127.0.0.1", 29942), timeout=5)) as sock:
            ...
            for _ in range(server_count):
                ...
                server_info = response[offset : offset + info_len].decode(
                    "utf-8", "replace"
                )
                offset += info_len
                ...
                if "Test Legacy Server" in server_info and "[5/10]" in server_info:
                    print(
                        f"✓ Legacy protocol test PASSED (registered server found: {server_info})"
                    )
                    return True

            print(
                f"✗ Legacy protocol test FAILED (server registered but with wrong info)"
            )
            return False
...
def run_tests():
    ...
    server_process = subprocess.Popen([sys.executable, "lobby.py"], cwd=repo_root)
